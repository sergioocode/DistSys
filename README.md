...
- CQRS en Products y Orders;
- MySQL como write store relacional independiente;
- MongoDB como read store documental independiente;
...
Clientes externos
  ...
  ▼
DistSys.API.Public
  ├── POST/PUT /order-ms/{**catch-all} → Orders Write API
  ├── GET      /order-ms/{**catch-all} → Orders Read API
  ├── /product-ms/{**catch-all}        → Products API
  └── /subscribe                       → RabbitMQ
...
├── src
│   ├── Api
│   │   ├── Public/DistSys.API.Public
│   │   └── Private/DistSys.API.Private
│   ├── Services
│   │   ├── Emails/DistSys.Services.Emails
│   │   ├── Orders
│   │   │   ├── DistSys.Services.Orders.Api.Write
│   │   │   ├── DistSys.Services.Orders.Api.Read
│   │   │   ├── DistSys.Services.Orders.BusinessLogic
│   │   │   ├── DistSys.Services.Orders.Consumer
│   │   │   └── DistSys.Services.Orders.Dtos
│   │   ├── Products
...
│   │   └── Subscriptions
│   ├── Shared
│   │   ├── API, Setup, Discovery, Secrets, Logging y Serialization
│   │   ├── Communication + RabbitMQ
│   │   └── Databases: MySQL y MongoDB
├── tools
├── docker-compose.yaml
└── DistSys.sln
```
...
| CQRS | APIs y persistencias separadas para comandos y consultas |
...
| Proyecto | Controller |
|---|---|
| Orders Write API | `OrderController` |
...
| `Orders Write API` | Proyección de pedido | `Orders.Consumer` | Actualización del read model de pedidos en MongoDB. |
...
|---|---|
| Orders Write API | Publica cambios de proyección de pedidos. |
...
MySQL es la fuente de verdad del write side de Products y Orders.
...
src/Services/Orders/DistSys.Services.Orders.BusinessLogic/DataAccess/OrdersWriteStore.cs
...
| Tabla | Uso |
|---|---|
| `Products` | Estado transaccional de productos. |
| `Orders` | Estado actual transaccional del pedido. |
| `OrderProducts` | Productos y cantidades de cada pedido. |
| `OrderStatusHistory` | Historial auditable de transiciones de estado. |

La conexión se construye usando Consul para host/puerto, Vault para credenciales y configuración para el nombre de la base.

### MongoDB

MongoDB contiene los modelos optimizados para consultas. No es la fuente de verdad de Products ni Orders.
...
src/Services/Orders/DistSys.Services.Orders.BusinessLogic/DataAccess/OrdersReadStore.cs
...
| `Orders` | Read model completo de pedidos. |
| `ProductName` | Cache local de nombres de producto usada por Orders. |
---
...
| Orders Write API | Crear/cambiar una orden en MySQL. | Cambio de proyección de la orden. |
...
## Orders: CQRS, historial de estados y read model

Orders usa la misma separación CQRS que Products:

| Proyecto | Responsabilidad |
|---|---|
| `DistSys.Services.Orders.Api.Write` | Crear pedidos y ejecutar cambios de estado. |
| `DistSys.Services.Orders.Api.Read` | Consultar pedidos y su estado desde MongoDB. |
| `DistSys.Services.Orders.BusinessLogic` | Casos de uso y stores de lectura/escritura. |
| `DistSys.Services.Orders.Consumer` | Transformar eventos en proyecciones de lectura. |
...
OrdersWriteStore
  ↓
MySQL: Orders + OrderProducts + OrderStatusHistory
  ↓
OrderProjectionChanged
  ↓
RabbitMQ / order.exchange
  ↓
Orders Consumer
  ├── enriquece nombres de Products
  └── actualiza OrdersReadStore
        ↓
MongoDB / Orders
  ↓
Orders Read API
...
`Version` evita que una entrega atrasada reemplace una proyección más reciente. El historial de estados se conserva en `OrderStatusHistory`, pero el estado actual se lee directamente desde `Orders`; por tanto, esta implementación no usa Event Sourcing.
---
...
La saga no reemplaza RabbitMQ ni CQRS. Los usa para coordinar procesos largos con consistencia eventual.
...
Los proyectos de pruebas se incorporarán en una fase posterior, con cobertura alineada con la arquitectura CQRS actual. Las carpetas de solución bajo Tests se conservan vacías para preparar esa fase sin presentar pruebas incompletas como funcionales.
...
No sustituye los conceptos principales del proyecto. API Gateway, mensajería, CQRS, idempotencia y consistencia eventual siguen siendo los mismos. Aspire solo puede simplificar la experiencia local de ejecución y observabilidad.
...
3. Levantar los microservicios base: Products Write/Read, Orders Write/Read, Subscriptions y Emails.
...
| `POST`, `PUT` | `/order-ms/{**catch-all}` | Proxy hacia Orders Write API. |
| `GET` | `/order-ms/{**catch-all}` | Proxy hacia Orders Read API. |
...
| `*` | `/reports/{**catch-all}` | Proxy hacia el read side de Orders. |

### Orders Write API

| Método | Ruta actual | Uso |
|---|---|---|
| `POST` | `/order/create` | Crear pedido en MySQL. |
...
| Método | Ruta actual | Uso |
|---|---|---|
| `GET` | `/order/{orderId}` | Obtener el read model del pedido. |
| `GET` | `/order/getorderstatus/{orderId}` | Obtener el estado proyectado. |
### Products Write API
...
| Graylog | Centralización de logs. |
| CQRS | Products y Orders separan write side en MySQL y read side en MongoDB. |
...
| Minimal APIs | Gateways, Products Read API y Orders Read API. |
...
| Rate limiting | `DistSysRateLimiterPolicy`. |
...
MongoDB como read store documental
