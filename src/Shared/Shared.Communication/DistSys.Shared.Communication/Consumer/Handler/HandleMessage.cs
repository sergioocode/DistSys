...
namespace DistSys.Shared.Communication.Consumer.Handler;
...
public class HandleMessage : IHandleMessage
{
    ...
    public Task Handle(IMessage message, CancellationToken cancellationToken = default)
    {
        ...
        List<IMessageHandler> handlers = _messageHandlerRegistry
            .GetMessageHandlerForType(handlerType, messageType)
            .ToList();
        ...
        foreach (IMessageHandler handler in handlers)
        {
            ...
            MethodInfo? handle = messageHandlerType
                .GetMethods()
