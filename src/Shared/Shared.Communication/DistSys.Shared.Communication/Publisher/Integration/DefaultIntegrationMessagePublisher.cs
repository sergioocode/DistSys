...
namespace DistSys.Shared.Communication.Publisher.Integration;
...
public class DefaultIntegrationMessagePublisher : IIntegrationMessagePublisher
{
    ...
    public DefaultIntegrationMessagePublisher(
        IExternalMessagePublisher<IntegrationMessage> externalPublisher
    )
    {
    ...
    {
        var integrationMessages = messages.Select(a =>
            IntegrationMessageMapper.MapToMessage(a, CalculateMetadata(metadata))
        );
