...
public interface IAggregateRepository<TAggregate>
{
    ...
    Task<TAggregate?> GetByIdOrDefaultAsync(
        Guid id,
        CancellationToken cancellationToken = default(CancellationToken)
    );
    Task SaveAsync(
    ...
    where TAggregate : Aggregate
{
    ...
    public async Task<TAggregate?> GetByIdOrDefaultAsync(
        Guid id,
        CancellationToken cancellationToken = default(CancellationToken)
    )
    {
        var events = (
            await _eventStore.GetEventsForAggregate(AggregateName, id, cancellationToken)
        ).ToList();
        if (!events.Any())
            return null;
