using DistSys.Shared.Databases.MySql;
using DistSys.Shared.Discovery;
using DistSys.Shared.Secrets;
...
{
    public static IServiceCollection AddMySql<T>(
        ...
        where T : DbContext
    {
        return serviceCollection
            .AddMySqlDbContext<T>(serviceProvider =>
                GetConnectionString(serviceProvider, databaseName)
            )
            .AddMysqlHealthCheck(serviceProvider =>
                GetConnectionString(serviceProvider, databaseName)
            );
    }
