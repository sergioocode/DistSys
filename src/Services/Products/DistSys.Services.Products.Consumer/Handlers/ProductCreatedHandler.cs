...
public class ProductCreatedHandler : IDomainMessageHandler<ProductCreated>
{
    ...
    public async Task Handle(
        DomainMessage<ProductCreated> message,
        CancellationToken cancelToken = default(CancellationToken)
    )
    {
        await _readStore.UpsertProductViewDetails(
            message.Content.Id,
            message.Content.ProductRequest.Details,
            cancelToken
        );
        await _readStore.UpdateProductStock(
            message.Content.Id,
            message.Content.ProductRequest.Stock,
            cancelToken
        );
        await _readStore.UpdateProductPrice(
            message.Content.Id,
            message.Content.ProductRequest.Price,
            cancelToken
        );

        await _integrationMessagePublisher.Publish(
            new ProductUpdated(message.Content.Id, message.Content.ProductRequest.Details),
            routingKey: "external",
